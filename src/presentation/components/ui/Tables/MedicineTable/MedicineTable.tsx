...
import { useMedicineTableController } from "./MedicineTable.controller";
import { MedicineDTO } from "@infrastructure/apis/client";
import DeleteIcon from '@mui/icons-material/Delete';
import { MedicineAddDialog } from "../../Dialogs/MedicineAddDialog";
import { useAppSelector } from "@application/store";

/**
 * This hook returns a header for the table with translated columns.
 */
const useHeader = (): { key: keyof MedicineDTO, name: string }[] => {
    const { formatMessage } = useIntl();

    return [
        { key: "medicineName", name: formatMessage({ id: "globals.medicineName" }) },
        { key: "medicineDescription", name: formatMessage({ id: "globals.medicineDescription" }) },
        { key: "medicinePrice", name: formatMessage({ id: "globals.medicinePrice" }) },
        { key: "medicineMeasurement", name: formatMessage({ id: "globals.medicineMeasurement" }) },
 ...
 */
const getRowValues = (entries: MedicineDTO[] | null | undefined, orderMap: { [key: string]: number }) =>
    entries?.map(
        entry => {
            return {
                entry: entry,
...
export const MedicineTable = () => {
    ...
    const { handleChangePage, handleChangePageSize, pagedData, isError, isLoading, tryReload, labelDisplay, remove } = useMedicineTableController(); // Use the controller hook.
    const rowValues = getRowValues(pagedData?.data, orderMap); // Get the row values.

    return <DataLoadingContainer isError={isError} isLoading={isLoading} tryReload={tryReload}> {/* Wrap the table into the loading container because data will be fetched from the backend and is not immediately available.*/}
        <MedicineAddDialog /> {/* Add the button to open the Medicine add modal. */}
